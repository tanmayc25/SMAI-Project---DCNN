...
net['pool1'] = lasagne.layers.Pool2DLayer(net['conv1'], pool_size=1)

net['conv2'] = lasagne.layers.Conv2DLayer(net['pool1'], num_filters=10, filter_size=5)
net['pool2'] = lasagne.layers.Pool2DLayer(net['conv2'], pool_size=1)
...
#Output layer:
net['out'] = lasagne.layers.DenseLayer(net['pool2'], num_units=output_size, 
...
import time
epochs = 10  #You can reduce the number of epochs to run it  faster (or run it for longer for a better model)
batch_size=100
